...
当约束执行器被挂载时，`bash` 公布配对的 `sandbox_permissions` 和 `justification` 字段。每次提示词组装时，`ctx.sandboxPolicy` 会按活跃会话投影这套封闭词汇：`read-only` 看到两个更宽目标，`workspace-write` 只看到 `danger-full-access`，`danger-full-access` 则看不到这两个字段。执行仍会拒绝任何注入或过时且不严格宽于该调用有效模式的目标。批准在执行之前解析。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。
...
- **按会话重新注册工具**：否决。注册表定义保持全局。策略归属方改为在提示词组装时投影分离后的 schema，既保留一套注册表词汇，也让每个请求只公布合法的更宽目标；模式切换会有意改变已记录的请求 header。
...
- **注册表 schema 保持静态，而请求 schema 是按会话投影。** 提示词组装会收窄或移除升级字段，不会修改已注册定义。执行时的严格放宽检查仍是注入或过时字段的安全边界，其错误会明确说明没有权限被拒绝，并要求去掉升级参数对后重试。
