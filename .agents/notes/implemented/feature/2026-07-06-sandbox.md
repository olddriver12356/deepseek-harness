...
When a confining executor is mounted, `bash` advertises paired `sandbox_permissions` and `justification` fields. At each prompt assembly, `ctx.sandboxPolicy` projects that closed vocabulary against the active session: `read-only` sees both wider targets, `workspace-write` sees only `danger-full-access`, and `danger-full-access` sees neither field. Execution still rejects any injected or stale target that is not strictly wider than that call's effective mode. Approval resolves before execution. `allowed-once` stamps the granted mode onto only that request, while `rejected`, `cancelled`, `unavailable`, a missing approval service, or a missing agent all fail closed with distinct results. No grant is persisted.
...
- **Re-register tools per session** — rejected: registry definitions remain global. The policy owner instead projects detached schemas during prompt assembly, preserving one registry vocabulary while letting each request advertise only legal wider targets; a mode switch deliberately changes the logged request header.
...
- **Registry schemas stay static while request schemas are per-session projections.** Prompt assembly narrows or removes the escalation fields without mutating registered definitions. The execution-time strict-wider check remains the safety boundary for injected or stale fields, and its error explicitly says nothing was denied and to retry without the escalation pair.
