...
Shipped Web surface 使用 `ui-shell`，并为持久 app 导航保留独立于会话面板几何状态的 72px 空间。两个拖动手柄的绝对位置都包含 rail 宽度，因此仍与可见的侧边栏和详情栏边界对齐。旧包仍可供显式 overlay 使用，但 generated catalog 描述 shipped default composition，因此不会包含故意重新启用 `ui-layout` 的 overlay。Active catalog sources 之间的重复声明仍然 fail closed。
