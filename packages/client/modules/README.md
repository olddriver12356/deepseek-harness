...
The same `/plugins` mount also serves a registered package's constrained binary assets: only simple `.png` and `.woff2` filenames directly under that package's `lib/assets/` directory. Unknown packages, nested names, path traversal, other extensions, and unreadable files return `404`; unsupported methods return `405`; every successful response is `no-cache`.
