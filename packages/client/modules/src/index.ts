 ...
 * combo scripts plus their source maps and constrained package assets,
...
export class ClientModuleRegistry extends Service {
  ...
  private readonly serveBundle = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    // Not an advertised combo response: fall back to a registered package's
    // constrained assets (lib/assets/*.png|*.woff2), read live since those are
    // small, infrequently-requested files outside the precomputed combo set.
    /* v8 ignore next -- `%(?:2f|5c)` guard: node:http already rejects a raw
       `%2f`/`%5c` in the request line before this handler ever sees it. */
    if (/%(?:2f|5c)/i.test(requestUrl.pathname) || PATH_TRAVERSAL.test(requestUrl.pathname)) {
      res.writeHead(404)
      res.end()
      return
    }
    ...
    try {
      ...
      res.writeHead(200, {
        'content-type': assetContentType ?? 'application/octet-stream',
