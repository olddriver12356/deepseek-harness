...
describe('client package assets', () => {
  ...
  it('serves a PNG owned by a registered package', async () => {
    ...
    const response = await routeRequest(route, `/plugins/${packageName}/assets/panel.png`)
  ...
  it('serves WOFF2 headers without a body for HEAD', async () => {
    ...
    const response = await routeRequest(route, `/plugins/${packageName}/assets/display.woff2`, 'HEAD')
  ...
  it('rejects unsupported methods', async () => {
    ...
    expect(await routeRequest(route, `/plugins/${packageName}/assets/panel.png`, 'POST'))
  ...
  it.each([
    ...
    expect(await routeRequest(route, url)).toMatchObject({ status: 404, body: Buffer.alloc(0) })
  })
})
