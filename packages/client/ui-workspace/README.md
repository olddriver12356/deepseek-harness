...
Global **New Session** clears the current selection so the user can choose a Workspace before a Session is created. A Workspace-specific New Session action opens a blank Session in that explicit Workspace. Initial application navigation still selects the recent Workspace.
