...
import {
  deriveFlat, deriveGroups, deriveSearchResults, owningGroupKey, recentWorkspace, UNGROUPED_KEY,
} from '../tree.ts'
...
export function WorkspaceBrowser({
  ...
  const recentWorkspaceId = useSessions(state => recentWorkspace(workspaces, state.byId))
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      ...
      <div className={css.listArea}>
        {wide && !(normalizedQuery !== '' ? sessionPhase === 'ready' : mainListReady) && (
          <div className={css.listSkeleton} role="status" aria-label={t('sessions.loading')}>
            <span /><span /><span /><span /><span />
          </div>
        )}
        {wide && (normalizedQuery !== '' ? sessionPhase === 'ready' : mainListReady) && (normalizedQuery !== ''
          ? (
            <SearchResults
