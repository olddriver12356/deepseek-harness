...
`design-platform.css` also owns the fixed `--dsw-specific-knowledge-*` art palette and display-font alias. The Knowledge package consumes those tokens for its Boujoy styling while keeping its component layout, collage asset, and font file package-local.
