...
The Web bundle mounts the Host News cache and the `ui-news` browser panel; its News cache is stored at `dshHomePath('news-cache')` and is read through the shared `api-remotes` assembly.
