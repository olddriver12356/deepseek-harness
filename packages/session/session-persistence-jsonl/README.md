...
On Windows, importing this backend does not load the POSIX-only `fs-ext` addon. The POSIX lease path loads it lazily when locking is required; Windows continues to use its native semaphore implementation. Skipping native installation scripts is not a substitute for installing `fs-ext` on POSIX.
