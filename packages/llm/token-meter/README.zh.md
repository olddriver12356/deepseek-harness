...
`contextOccupancy(pressure)` 从浏览器安全的 `/client` 入口导出，供 Conversation、Shell 和 Monitor 显示占用率。它优先采用 projected tokens，其次采用已报告的 pressure；用量或容量尚未就绪时返回 `null`，百分比四舍五入并以 100% 为显示上限。消费方共享这个纯计算，无需导入另一个 UI 插件的实现。
