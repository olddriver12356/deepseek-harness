...
`contextOccupancy(pressure)` is exported from the browser-safe `/client` entry for Conversation, Shell, and Monitor displays. It prefers projected tokens over reported pressure, returns `null` until usage and capacity are known, and rounds the percentage with a 100% display cap. Consumers share this pure calculation without importing another UI plugin's implementation.
